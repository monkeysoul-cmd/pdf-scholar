...
export default function Auth() {
  ...
  return (
    <div className="min-h-screen w-screen overflow-y-auto bg-dot-grid flex flex-col items-center justify-center p-4 md:p-8 select-none relative" id="auth-view">
      ...
      >
        {/* Top border accent */}
        <div className="absolute inset-x-0 top-0 h-[2px] bg-gradient-to-r from-transparent via-emerald-400 to-transparent shadow-[0_0_15px_rgba(16,185,129,0.5)]" />
        ...
        <div className="flex items-center justify-center gap-2 mb-8">
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/5 border border-white/10 text-white/80 text-[10px] font-mono font-medium uppercase tracking-wider shadow-inner">
            <VectorAIIcon className="w-3 h-3 text-emerald-400 drop-shadow-[0_0_5px_currentColor]" /> Vector Academic RAG
          </span>
        ...
        <div className="text-center mb-8 flex flex-col items-center">
          <motion.div
            ...
            className="w-16 h-16 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center text-emerald-400 mb-4 shadow-inner"
          >
            <PDFScholarLogo className="w-8 h-8 text-emerald-400 drop-shadow-[0_0_12px_currentColor]" />
          </motion.div>
          
          <h1 className="flex items-center justify-center gap-2 font-black text-3xl sm:text-4xl tracking-tight leading-none">
            <span className="text-emerald-400 drop-shadow-[0_0_15px_rgba(16,185,129,0.4)]">PDF</span>
          ...
          >
            ...
            {message && (
              <motion.div
                initial={{ opacity: 0, y: -10 }}
                animate={{ opacity: 1, y: 0 }}
                className="bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 rounded-xl p-3.5 text-xs font-mono uppercase text-center leading-relaxed backdrop-blur-sm"
              >
                {message}
              </motion.div>
            )}
            ...
            <div className="space-y-3.5">
              <div className="relative group">
                <span className="absolute inset-y-0 left-0 pl-4 flex items-center text-zinc-500 group-focus-within:text-emerald-400 transition-colors">
                ...
                <input
                  ...
                  className="w-full bg-black/40 border border-white/10 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500/30 hover:border-white/20 text-white text-xs px-4 py-3.5 pl-12 rounded-xl transition-all outline-none placeholder-zinc-500 font-medium shadow-inner"
                  required
                />
              </div>
        ...
        <div className="mt-8 text-center border-t border-white/10 pt-6">
          ...
          >
            <Zap className="w-3.5 h-3.5 text-emerald-400 drop-shadow-[0_0_5px_currentColor]" />
            {isLogin ? <span>New to <strong className="text-emerald-400">PDF</strong> Scholar? Create account</span> : <span>Already have an account? Sign In</span>}
          </button>
        </div>
      </motion.div>
    </div>
