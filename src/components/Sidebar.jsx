...
export default function Sidebar() {
  ...
  const SidebarInner = ({ collapsed }) => (
    <aside
      ...
      } bg-black/40 backdrop-blur-2xl text-white border-r border-white/5 flex flex-col h-full select-none relative shadow-2xl transition-all duration-300 ease-in-out shrink-0 z-50`}
      id="sidebar-container"
    >
      <div className="absolute inset-0 bg-noise opacity-50 pointer-events-none" />

      <div
        className={`border-b border-white/5 relative z-10 ${
            ...
            return (
              <motion.button
                ...
                className={`w-full relative flex items-center ${
                  collapsed ? "justify-center px-0 py-3 rounded-xl" : "justify-between px-3 sm:px-3.5 py-3 rounded-lg"
                } text-xs font-bold uppercase tracking-wider text-left transition-all ${
                  isActive
                    ? "text-black bg-gradient-to-r from-[#00FF66] to-[#00e55b] shadow-[0_0_15px_rgba(0,255,102,0.3)]"
                    : isDisabled
            ...
            {activeDoc ? (
              collapsed ? (
                <motion.button
                  ...
                  className="w-full p-3 glass-card border-[#00FF66]/50 rounded-xl flex items-center justify-center text-[#00FF66] relative shadow-[0_0_15px_rgba(0,255,102,0.1)] cursor-pointer"
                >
                ...
                >
                  <div className="flex items-start gap-3 relative z-10">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="text-[10px] font-mono text-zinc-400 mt-1 flex items-center gap-1.5">
                        <span>{activeDoc.pageCount} Pgs</span>
                        <span>•</span>
                        <span>{activeDoc.chunkCount} Chnks</span>
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 mt-1 relative z-10">
                    <motion.button
                      ...
                      whileTap={{ scale: 0.98 }}
                      onClick={() => setTab("chat")}
                      className="flex-1 text-center bg-gradient-to-r from-[#00FF66] to-[#00e55b] hover:from-[#00e55b] hover:to-[#00cc55] text-black font-extrabold text-[10px] uppercase tracking-wider py-2 rounded-lg transition-all shadow-[0_0_12px_rgba(0,255,102,0.2)] cursor-pointer"
                    >
                      Start Chat
                    </motion.button>
                    <motion.button
      ...
      <div className={`${collapsed ? "p-2 sm:p-3" : "p-4 sm:p-5"} border-t border-white/5 flex flex-col gap-3 text-xs font-mono uppercase tracking-wider shrink-0 relative z-10`}>
        {!collapsed ? (
          <>
      ...
      <AnimatePresence>
        {!isMobileOpen && (
          <motion.button
            ...
            className="md:hidden fixed top-3 left-3 z-40 w-10 h-10 glass-card border-white/10 text-zinc-300 hover:text-[#00FF66] hover:border-[#00FF66]/40 rounded-lg flex items-center justify-center transition-all shadow-lg"
          ...
          <>
            ...
            >
              <div className="relative h-full shadow-2xl">
                ...
                <button
                  onClick={() => setIsMobileOpen(false)}
                  className="absolute top-4 right-4 p-2 glass-card hover:bg-white/10 text-zinc-400 hover:text-white rounded-full transition-all z-50"
                >
                  <X className="w-4 h-4" />
                </button>
              </div>
